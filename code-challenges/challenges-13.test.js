...
let eachOneLength=arr.map(str=>str.length);
let longest= eachOneLength.sort((a,b)=>(a<b)? 1:-1)

return arr.reduce((acc,val,i)=>{
  if(val.length==longest[0]){
    return acc=i}else 
    return acc
},-1);
};
...
console.log(firstLetters(['ajajaj','jj','jaj','ggggtyynnn']))
...
const allHappy = (arr) => {
let unHappy=arr.filter(val=> !(val.includes(":)")));
let status=(unHappy.length>0)? false:true;
return status;
};
console.log(allHappy(['dll']))
console.log(allHappy(['dll:)']))
