...
const isCapitalized = (str) => {
  // Solution code here...
  let regexW = /\b[A-Z]\w+/g;
  let arr = [];
  if (str.match(regexW)) {
    arr = str.match(regexW)
  }

  return arr;
};
console.log(isCapitalized('these words are all failures'));
...
console.log(citiesAtoJ(['Cleveland', 'San Diego', 'Birmingham', 'Seattle', 'Miami', 'New York City', 'Omaha', 'Portland', 'Austin', 'Boston', 'Newport Beach', 'Hoboken']))
...
const matchMonth = (input) => {
  let oct = ['October', 'Oct', 'october', 'oct']
  oct.forEach(element => {
    if(element===input){
